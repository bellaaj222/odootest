...
class MyDiagramView(DiagramView):
    ...
    def get_diagram_info(self, id, model, node, connector,
        ...
        connector_model = request.env[connector]
        ...
        fields = request.env['ir.model.fields']
        field = fields.search([
            ('model', '=', model),
            ('relation', '=', node),
            ('ttype', '=', 'one2many')
        ])
        node_act = request.env[node]
        ...
        _id, name = request.env[model].browse([id]).name_get()[0]
        ret = dict(
            nodes=nodes,
            conn=connectors,
            display_name=name,
    ...
    @http.route(
        '/web_diagram_position/diagram/eval', type='json', auth='user')
    ...
    @http.route(
        '/web_diagram_position/diagram/eval_color', type='json', auth='user')
    ...
    @http.route(
        '/web_diagram_position/diagram/eval_label', type='json', auth='user')
