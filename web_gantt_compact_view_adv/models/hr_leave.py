...
class HolidaysRequest(models.Model):
    ...
    leave_link_ids = fields.One2many('hr.leave.link', 'source_id', string="Order Links")
    ...
    def search_read_links(self, domain=None):
        datas = []
        tasks = self.env['hr.leave'].search(domain)
        for task in tasks:
...
class HolidaysRequestLink(models.Model):
    _name = "hr.leave.link"
    _description = "Leave Report Calendar Links"

    source_id = fields.Many2one('hr.leave', string='Leave')
    target_id = fields.Many2one('hr.leave', string='Target Leave', required=True)
