...
import math
_logger = logging.getLogger('base.permis.merge.automatic.wizard')
class MergePermisLine(models.Model):

    _name = 'base.permis.merge.line'
    _order = 'min_id asc'

    wizard_id = fields.Many2one('base.permis.merge.automatic.wizard', 'Wizard')
...
class EbMergePermis(models.Model):
    ...
    _description = 'Merge permiss'
    _rec_name = 'name'
    
    @api.model
    def default_get(self, fields):
        res = super(EbMergePermis, self).default_get(fields)
...
##                                         _('You cannot validate this journal entry because account "%s" does not belong to chart of accounts "%s"!') % (tt, tt))
        ...
        if self.env.context.get('active_model') == 'project.task.work' and active_ids:
            ...
            ##wiz_id=self.env['base.invoice.merge.automatic.wizard'].create({'work_ids': active_ids}) 'wizard_id': wiz_id,
            ##res.update({})
            
            work=self.env['project.task.work'].browse(active_ids[0])
            if 'Demande' not in work.name:
                raise osv.except_osv(_('Action Impossible!'),_("Seules les demandes de permis sont à sélectionner!"))
##                    list.append (jj.product_id.id)
           ## raise osv.except_osv(_('Transfert impossible!'),_("Pas de Stock suffisantdffd pour l'article %s  !")% work.date_start)
##            task=self.env['project.task'].search( [('project_id','=',work.project_id.id),('product_id.default_code','like','5'),('product_id.name','like','Permis')])
##            emp=work.employee_id.id
##
##            for jj in task:
##               list.append (jj.id)
##               res['project_id'] = jj.project_id.id
##               res['product_id'] = jj.product_id.id
##               res['date_start'] = work.date_start
##               res['date_end'] = work.date_end
##               res['employee_id'] = emp
##            res['work_ids'] = list
            for jj in active_ids:
                work=self.env['project.task.work'].browse(jj)
               ## self.env['project.task.work'].write( {'state': 'affect' })
                ...
                if work.kit_id:
##                    self._cr.execute('update project_task_work set  kit_id=%s where id=%s ' , (work.kit_id.id,work.id))
##                    self._cr.execute('update project_task_work set  product_id=%s where id=%s ' , (165,work.id))
                    res.update({'project_id':work.project_id.id,'zone':work.zone,'secteur':work.secteur,'employee_id':work.employee_id.id or 30})##,'kit_id':work.kit_id.id,'product_id':165
                
                else:
                    res.update({'project_id':work.project_id.id,'zone':work.zone,'secteur':work.secteur,'employee_id':work.employee_id.id or 30})
...
##    work_id= fields.Many2one('project.task.work', string='Wizard')
    ...
    work_ids = fields.Many2many('project.task', string='Works')#'merge_tasks_rel', 'merge_id', 'task_id',)
...
##                tag.write({'tag_ids': (6, 0, [self.dst_task_id.id])})
        ...
        #write the Assiged TO user_id
            ...
            raise Warning('There is no user assigned to the merged task, and the destination task doesn''t have assigned user too!!!')
    ...
    #your changes
        ...
        for current in self.browse(cr, uid, id, context=context):
            ...
            ##if not default.get('name'):
                ...
                cr.execute('select sequence from project_task_work where task_id=%s and sequence is not Null order by sequence desc limit 1', (tt.id,))
                res = cr.fetchone()
                
                if res:
                    sequence=res[0]+1
                sequence=60001
                kk=self.pool.get('project.task.work').search(cr, uid,[('project_id','=',tt.project_id.id),('sequence','=',sequence)])
                
                while kk:
                    sequence=sequence+1
                    kk=self.pool.get('project.task.work').search(cr, uid,[('project_id','=',tt.project_id.id),('sequence','=',sequence)])
                ##raise osv.except_osv(_('AYMEN!'),_("JE SUIS LA !"))
                if tt.product_id:
                    cte=self.pool.get('project.task.work').create(cr, uid, {
                        'task_id': tt.id,
    ...
    def show_results(self, cr, uid, ids, context=None):
       ...
       ## raise osv.except_osv(_('Transfert impossible!'),_("Pas de Stock suffisant pour l'article %s  !")%project_id )
        ...
        if current.project_id:
            ...
            cr.execute('select id from project_task_work where project_id= %s and date_start>=%s and date_start <=%s and state in %s and etape in %s', ( current.project_id.id,current.date_from,current.date_to,('draft','affect'),tuple(l)))
            pp= cr.fetchall()
       ...
       ## raise osv.except_osv(_('Transfert impossible!'),_("Pas de Stock suffisant pour l'article %s  !")%project_id )
        ...
        if current.project_id:
            ...
            for s2 in current.line_ids1:
                if s2.employee_id:
                    cr.execute("INSERT INTO project_task_work (task_id,categ_id,product_id,name,uom_id,date_start,date_end," \
                              ...
                              " ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s',%s,%s,'%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')"
                               %(s2.task_id.id,s2.categ_id.id,s2.product_id.id,s2.name,s2.uom_id.id,
                                 s2.date_start,s2.date_end,s2.poteau_i,s2.poteau_t,int(s2.color),s2.hours,s2.total_t,s2.project_id.id,s2.gest_id.id ,
                                 s2.employee_id.id ,s2.uom_id.id,s2.etape,'affect',s2.zone,s2.secteur,s2.sequence, 'Zone '+s2.zo,'Secteur '+s2.sect,s2.task_id.coordin_id.id or False,s2.task_id.coordin_id.id or False,s2.gest_id.id or False,s2.task_id.reviwer_id1.id or False,s2.task_id.coordin_id1.id or False,s2.task_id.coordin_id2.id or False,s2.task_id.coordin_id3.id or False,s2.task_id.coordin_id4.id or False,s2.project_id.partner_id.id or False))
                else:
                    cr.execute("INSERT INTO project_task_work (task_id,categ_id,product_id,name,uom_id,date_start,date_end," \
                              ...
                              " ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s',%s,'%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')"
                               %(s2.task_id.id,s2.categ_id.id,s2.product_id.id,s2.name,s2.uom_id.id,
                                 s2.date_start,s2.date_end,s2.poteau_i,s2.poteau_t,int(s2.color),s2.hours,s2.total_t,s2.project_id.id,s2.gest_id.id ,
                                 s2.uom_id.id,s2.etape,'draft',s2.zone,s2.secteur,s2.sequence, 'Zone '+s2.zo,'Secteur '+s2.sect,s2.task_id.coordin_id.id or False,s2.task_id.coordin_id.id or False,s2.gest_id.id or False,s2.task_id.reviwer_id1.id or False,s2.task_id.coordin_id1.id or False,s2.task_id.coordin_id2.id or False,s2.task_id.coordin_id3.id or False,s2.task_id.coordin_id4.id or False,s2.project_id.partner_id.id or False))
 ##               else:
