...
class EbMergeTasks(models.Model):
    ...
    def show_results(self):
        ...
        if current.project_id:
            if current.type == '1':
                ...
                if len(v) > 0:
                    self.env.cr.execute(
                        'SELECT id FROM project_task_work WHERE project_id= %s AND kit_id IN %s AND active=TRUE AND is_copy=FALSE',
                        (current.project_id.id, tuple(v)))
                    ll = self.env.cr.fetchall()
                else:
                    self.env.cr.execute(
                        'select id from project_task_work where project_id= %s and etape in %s',
                        (current.project_id.id, tuple(f)))
        ...
        if current.project_id:
                        ...
                        if s2.task_id.kit_id:
                            query = """
                                INSERT INTO project_task_work (
                                    kit_id, task_id, categ_id, product_id, name, uom_id, date_start, date_end,
                                    ...
                                    gest_id3, current_gest, current_sup, partner_id, work_orig, pr_project_id, is_copy
                                )
                                VALUES (
                            ...
                            values = (
                                ...
                                s2.poteau_i, s2.poteau_t, int(s2.color), s2.hours, s2.total_t, s2.project_id.id,
                                s2.gest_id.id or None, s2.uom_id.id, s2.etape, 'draft',
                                s2.zone, s2.secteur, seq, True, current.id, s2.is_display,
                                'Zone ' + s2.zo, 'Secteur ' + str(s2.secteur).zfill(2),
            ...
            if len(current.work_ids) > 0:
                ...
                if len(v) > 0:
                    ...
                    for kk in res_cpt:
                        task_work = self.env['project.task.work'].browse(kk)
                        task_work.write({'w_id': current.id})
                        if task_work.state in ('draft', 'affect'):
                            task_work.write({'active': False})
    ...
    def cancel_(self):
        ...
        if found:
            for work in found:
                raise UserError(
                    _("Attention!\nAn assignment is made with this configuration: Project %s, Zone: %s, Secteur: %s, "
                      "Intervenant: %s") % (
                        current.project_id.npc, work.zone, work.secteur, work.employee_id.name))
        to_delete_ids = self.env['project.task.work'].search([('w_id', '=', current.id), ('is_copy', '=', True)]).ids
        for to_delete_id in to_delete_ids:
            self.env['project.task.work'].browse(to_delete_id).unlink()

        archived_ids = self.env['project.task.work'].search([('w_id', '=', current.id), ('active', '=', False)]).ids
        if archived_ids:
            for archived_id in archived_ids:
                self.env['project.task.work'].browse(archived_id).write({'w_id': False, 'active': True})
        ...
        if self.project_id:
            ...
            for task in tt:
                if self.project_id.is_kit and task.kit_id.id not in ltask1:
                    ...
                    if 'Etape' in task.product_id.name and task.product_id.name not in ltask1:
